...
public class Demon : PhaseMonster
{
    ...
    [SerializeField] private DemonProjectileAttack projectile; // 원거리 공격
    [SerializeField] private Transform projectileAttackPoint; // 원거리 공격 시작 위치
    ...
    private void ChangeNextRandomAttack() // 다른 타입의 공격을 준비한다
    {
    ...
    protected override async UniTaskVoid BattleMode()
    {
        ...
        async UniTaskVoid NextAttackDelay()
        {
            ...
            {
                ChangeNextRandomAttack(); // 다른 타입의 공격을 준비한다
                state.ChangeState(StateType.Move);
            }
