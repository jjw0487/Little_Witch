...
public class InventoryData
{
    public InventoryData(ItemDataContainer _dataContainer)
    {
        ...
        LoadSavedData(); // PlayerPrefs 에 저장된 슬롯 데이터를 로드
    }

    private ItemDataContainer dataContainer;

    // 소지품의 총 수량을 저장 ( 퀵슬롯 아이템 사용 이벤트, 아이템 스왑등으로 동일 아이템이 여러 슬롯으로 분리될 시 대비를 위해 ) 
    ...
    public int Gold
    {
        ...
        set
        {
            ...
            {
                EventManager.goldNotificationEvent(value); // 골드 획득 UI 이벤트 
            }
    ...
    public bool FindEmptySlot(int _itemId, int _val) // 새롭게 추가될 아이템을 위해 빈 공간을 전달
    {
        for(int i = 0; i < inventorySlot.Length; i++) 
        {
    ...
    public bool FindSameItemSlot(int _itemId, int _value) // 같은 아이템이 존재한다면 해당 슬롯을 전달
    {
        for (int i = 0; i < inventorySlot.Length; i++)
        {
    ...
    public bool CheckItemValue(int _itemId, int _value) // 아이템이 필요 수량보다 더 존재하는지 확인
    {
        if (possession.TryGetValue(_itemId, out Collected col))
        {
    ...
    public bool UseItem(int _itemId, int _value) // 퀘스트, 기타 등에서 사용처리 해야하는 경우
    {
    ...
    public bool SellItem(int _itemId, int _value) // 상점에 판매하는 경우
    {
        if (possession.TryGetValue(_itemId, out Collected col))
        {
            col.value -= _value;
    ...
    public void ThrowItemAway(Vector3 pos, int itemId, int itemValue) // 아이템을 버렸을 때
    {
