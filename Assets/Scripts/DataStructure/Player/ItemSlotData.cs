...
{
    public ItemSlotData(SlotType _type, int _slotIdx) // 데이터가 없을 시
    {
        slotIdx = _slotIdx;
        slotType = _type;
    }
    ...
    public QuickItemSlot connectedQuickSlot; // 연결된 퀵슬롯 UI
    ...
    public void Swap(ItemData _data, int _value) // 아이템 위치 교체
    {
        if (_value <= 0) Remove();
        else Acquire(_data, _value);
    }

    public void Acquire(ItemData _data, int _value) // 새로운 아이템 획득
    { 
        data = _data;
        value = _value;
    ...
    public void Increase(int _value) // 아이템 수량 증가
    { 
        value += _value;
    ...
    public void Decrease(int _value) // 아이템 수량 감소 ( 상점 이용 )
    { 
        value -= _value;
    ...
    public void UseItem(int _value) // 아이템 수량 감소 ( 사용 )
    {  
